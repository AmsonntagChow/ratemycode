...
vibe 出来的应用，哪些功能真能用、哪些不能，作者自己基本不知道。这个 Skill 给它做一次 Staff 级产品审查。
...
完整程度的审查会运行你的产品，把故障追溯到实现，并重新运行检查。它消耗的时间和 token 是快速体检的数倍。

如果你的套餐是按量计费或固定配额——大多数 Claude Code、Codex 和 Cursor 订阅都是——这个成本会挤占你的实际工作。所以快速体检是默认选项。它把审查集中在少数几项检查上。这几项检查最常决定一个东西能不能安全发布。做完就停。

你放弃的是**保证的下限**——覆盖到的东西更少。报告本身的诚实度没有降低。每个被发现的问题仍然完整报告。所有没覆盖到的东西，都会明确标为未知，并附上一个能解决它的测试。

什么情况下升级？当风险确实需要时：真实资金、私有数据、公开用户，或者一个你无法回退的决定。
