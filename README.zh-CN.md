...
```text
1. 角色：产品负责人 / 挑剔用户 / 系统/后端 Staff 工程师 / 前端 Staff 工程师 / 怀疑型 VC / 答辩老师
...
| 系统/后端 Staff 工程师 | 深入审查不变量、安全边界、并发、故障处理、运维和变更安全性 |
| 前端 Staff 工程师 | 测试真实浏览器行为、异步状态、无障碍、性能、响应式支持、组件体系和交互细节 |
...
以前端 Staff 工程师的身份，按上线门禁审查这个 Dashboard。用真实浏览器检查键盘和读屏器操作、异步状态、移动端布局、性能和交互质量。
...
开头会把所有已确认问题——未处理、处理中、用户接受、已独立验证修复，或当前修复无法验证——放进同一份按严重程度排序的列表；每项严格使用一句通俗语言，说明会发生什么以及为什么重要。它绝不会用“前三项”的上限隐藏其他问题。当前修复无法验证的问题还会在 `待验证` 中再次出现；仍开放的待验证项和生效中的流程阻塞也保留在那里，不会被写成已经解决的事实。随后，四条证据通道会准确展示哪些内容已被证实；一条绿色通道不能替另一条兜底。详细问题会形成完整闭环：从产品不变量，到精确复现步骤、可见证据、后果、最低限度修复方案和验收测试。缺失或过期的证据绝不算作通过。
