...
做它的初衷，是想让更多有想法的人更容易把想法落地成能用的东西。
...
在询问角色和程度之前，RateMyCode 会先做一次只读的最小范围检查，确认审查对象是什么。它要确认一个连贯的产品目标，并把关联的仓库、部署地址、日志、数据和文档都当成同一个产品的证据，不会误判成多个项目。

如果当前工作区为空，或与产品无关，或指定目标不存在，或同时有多个互不相关的产品却无法确定目标，它会请你提供项目路径、仓库、部署地址、附件或产品证据文件。然后它停止。它不会检查内容，不会评分，不会给出结论。

项目存在但跑不起来时，它仍然会往下走，但只做明确受限的静态审查。

## 角色与程度

确认唯一审查目标后，这个 skill 会在开始前问两个问题；请求里已经写了的，就不再问。
...
完整审查会真的把你的产品跑起来，把故障一路追到实现代码里，然后重跑检查确认。它花的时间和 token，是快速体检的好几倍。

所以先看你是哪档：**Claude Code Pro 用户，省着点用**——一次完整审查能啃掉你今天写代码的那份额度；**Max 用户放心跑**。Codex、Cursor 的包月套餐也得掂量。默认走快速体检：只查那几项最常决定“能不能安全发布”的，查完就停。

少的是覆盖面，不是报告的诚实度。查出来的问题一条不落全报；没查到的部分记成明确的未知，并给出唯一能定它的那一个测试。

什么时候升级到完整审查：涉及真金白银、私密数据、真实用户，或者一个改不回来的决定——这时候才值得花那份额度。
...
开头的列表包含所有已确认的问题，未处理的、处理中的、你接受了风险的、已独立验证修复的，全都在里面。所有问题按严重程度排序。每一项用一句通俗语言说明会发生什么，以及为什么重要。

它不会用“前三项”的上限隐藏其他问题。无法验证的修复、还没解决的未知项、正卡着流程的问题，都留在 `Pending verification` 下面。它们不会被写成已经解决的事实。

四条证据通道展示哪些内容已被证实。一条绿色通道不能替另一条兜底。每条问题都从头串到尾：从它破坏了哪条产品不变量，到精确的复现步骤、看得见的证据、会造成什么后果、最小的修法，以及验收它的测试。缺失或过期的证据，绝不算通过。

多份文档描述同一件事时，默认比对的是事实，不是文字是否逐字相同。措辞可以不同。但同一版本、同一环境里，接口、权限、默认值、限制、有效期、错误和生命周期规则必须兼容。只有用户明确要求，或生成镜像契约规定必须完全相同时，才进行逐字节比较。
...
复测结果是 `unverifiable` 的，留在待验证里，不会被写成已经修好了。
...
只有当 `deployment_coverage` 明确说明已检查完整部署范围，并确认没有别的层已经在挡这件事时，完整的代码或文档检查才可以触发门禁。它不能关闭门禁，不能清除未知项，不能证明修复。E0 主张永远不算证据。
